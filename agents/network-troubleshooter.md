...
tools: mcp__netauto__net_list_devices, mcp__netauto__net_device_facts, mcp__netauto__net_run_show, mcp__netauto__net_get_config, mcp__netauto__net_discover_local, mcp__netauto__net_scan_ports, mcp__netauto__net_connect_adhoc, mcp__netauto__net_topology, mcp__netauto__net_config_diff, Bash, Read
---
...
2. Narrow the path. `net_topology` shows which devices are actually adjacent,
   ...
   management is closed or filtered" before you go looking further. When the
   fault involves a host with no inventory entry, `net_connect_adhoc` reads it
   directly -- an undocumented switch in the path is a common enough cause that
   not being able to look at it is its own dead end.
